...
describe("nextHeaderHidden", () => {
  ...
  test("reveals on a clear scroll up", () => {
    expect(
      nextHeaderHidden({ ...base, hidden: true, y: 460, lastY: 500 }),
    ).toBe(false);
  });
  ...
  test("keeps current state within the jitter threshold", () => {
    ...
    expect(
      nextHeaderHidden({ ...base, hidden: true, y: 500 + wobble, lastY: 500 }),
    ).toBe(true);
    expect(
      nextHeaderHidden({ ...base, hidden: false, y: 500 - wobble, lastY: 500 }),
    ).toBe(false);
  });

  test("never hides near the top of the page", () => {
    expect(nextHeaderHidden({ ...base, hidden: true, y: 100, lastY: 60 })).toBe(
      false,
    );
